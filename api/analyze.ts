...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!summary) {
    return res.status(400).json({ error: "Summary dataset tidak ditemukan dalam body request." });
  }

  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    return res.status(400).json({ error: "API Key Gemini tidak dikonfigurasi pada server Vercel. Silakan atur variabel lingkungan GEMINI_API_KEY di dashboard Vercel." });
  }
  ...
  try {
    ...
    if (!responseText) {
      throw new Error("Tidak ada respons teks yang dikembalikan oleh model");
    }
    ...
    return res.status(500).json({ error: "Gagal memproses analisis di backend Vercel: " + error.message });
  }
}
